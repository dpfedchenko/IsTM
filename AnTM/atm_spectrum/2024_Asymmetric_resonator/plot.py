...
def T_spectrum_plot(T, A, R):
    spectrum_atm = []
    # ATM
    plt.plot(lmbd[::-1], T[::-1], linewidth = 9, c = '#99EE6B', label = 'ATM_T')
    plt.plot(lmbd[::-1], R[::-1], linewidth = 9, c = '#FA7080', label = 'ATM_R')
    plt.plot(lmbd[::-1], A[::-1], linewidth = 9, c = '#64AAD0', label = 'ATM_A')
