...
from build import P, Dm, Di, R1m, R2i, calculate_transmittance, LR_sub, lmbd
...
for i in range(NK + 1):
    ...
    Tf_CLC1 = P(Fo_CLC1, Fe_CLC1) @ Di(LR_CLC1[2], LR_CLC1[1]) @ R2i(LR_CLC1[4] + delta_CLC1) @ R1m(LR_CLC1[4]) @ Dm(LR_CLC1[2], LR_CLC1[1]) @ P(Fe_CLC1, Fo_CLC1)
    
    Fe_CLC2 = np.exp(1j * 2 * np.pi * K * LR_CLC2[0] / N_sub / 2 * LR_CLC2[1])
    Fo_CLC2 = np.exp(1j * 2 * np.pi * K * LR_CLC2[0] / N_sub / 2 * LR_CLC2[2])
    Tf_CLC2 = P(Fo_CLC2, Fe_CLC2) @ Di(LR_CLC2[2], LR_CLC2[1]) @ R2i(LR_CLC2[4] + delta_CLC2) @ R1m(LR_CLC2[4]) @ Dm(LR_CLC2[2], LR_CLC2[1]) @ P(Fe_CLC2, Fo_CLC2)
